...
test('FreezeHost 自动续期', async () => {
    ...
    try {
        ...
        try {
            ...
            await page.waitForURL(/free\.freezehost\.pro/, {
                timeout: 15000,
            });
        ...
        try {
            await page.waitForURL(
                (url) =>
                    url.includes('/callback') || url.includes('/dashboard'),
                { timeout: 10000 }
            );
        } catch {}
        ...
        if (renewalStatusText) {
            ...
            const remainingDays = daysMatch
                ? parseFloat(daysMatch[1])
                : null;
        ...
        const externalLinkIcon = page
            .locator('i.fa-external-link-alt, i.fas.fa-external-link-alt')
            .first();

        await externalLinkIcon.waitFor({
            state: 'attached',
            timeout: 15000,
        });
